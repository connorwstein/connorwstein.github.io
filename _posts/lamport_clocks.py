...
We can use threads to emulate the ideas.

Each thread has a logical clock which is incremented after each 
event.

If each process satisfies the clock condition, then the events can 
be ordered globally. 
a -> b (partial ordering)
a => b (global ordering)

Solution to the synchronization problem of sharing a single resource
among multiple processes. Conditions to be satisfied:
    1. Process which has been granted the resource must release it before it can be granted to another process
    2. Different requests for the resource must be granted in the order in which they are made
    3. If every process which is granted the resource eventually releases it, then every request
    will eventually be granted

Each process maintains its own request queue which is never seen by any other process.    
"""
import signal
import sys
import time
...
class Process(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            if not self.requested:
                ...
                self.lamport_clock += 1
                # Check after processing if the resource is available for me now, if so, grab it
                # We need earliest request to be ours and check that we have received an older message
                # from everyone else 
    ...
    def check_available(self):
        ...
        if our_req is None:
            return False
        # We found our req make sure it is younger than all the others and we have an older one from 
        # the other guys
        for req in self.request_queue:
...
try:
    ...
    t3.start()
    while True:
        # Need some arbitrary timeout here, seems a bit hackish. If we don't do 
        # this then the main thread will just block forever waiting for the threads to return
        # and the keyboardinterrupt never gets hit. Interestingly regardless of the 
        # timeout, the keyboard interrupt still occurs immediately upon ctrl-c'ing
        t1.join(100)
        t2.join(100)
        t3.join(100)
